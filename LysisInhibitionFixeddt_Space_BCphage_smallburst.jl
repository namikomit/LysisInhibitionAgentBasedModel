...
function simulate_space_agents(states::Vector{SState}, time_step, record_time_step, final_time, 
    ...
    while timenow < final_time
        ...
        #phage BC; if phage is farther than size_colony from the edge of the colony, it is removed.
        # Find indices of nonzero elements in an array
        inds = findall(states -> states.Bstate != 0, states)
        if !isempty(inds)
            min_index = minimum(inds)
            max_index = maximum(inds)
        else
            min_index = 0
            max_index = 0
        end
        size_colony=max_index-min_index+1
        for i in 1:lattice_size
            if states[i].Phage > 0
                if (i < min_index - size_colony) || (i > max_index + size_colony)
                    states[i].Phage = 0
                end
            end
        end
        ...
        if timenow - time_series[end] >= record_time_step
            ...
            if non_zero_Bstate_count==0
                ...
                timenow=final_time
                break
            elseif non_zero_Bstate_count> 149
                println("colony size reached 150")
                timenow=final_time
                break
            end
        end
...
global lattice_size=500
global push_distance=50

global time_step=min(0.2/hop_rate, 1/eta)
if(time_step*hop_rate>0.5)
    println("Hop rate is too high")
end 
figures_dir = "figure_files_space_phageBC_smallburst"
mkpath(figures_dir)
...
for(i, (binary_Btimeseries_2d, condition_title)) in enumerate(all_binary_Btimeseries_2d)
    ...
    binary_Btimeseries_2d[:, 190:310],
    yflip = true, 
    xlabel = "position",
    ylabel = "time",
    color = [:white, :orange, :blue, :green], 
    title = condition_title,
     xticks = ([11, 61, 111], ["200","250","300"]),
    yticks = (1:20:101, 0:20:100),
    colorbar = false,
    ...
    bar_plot = bar(1:length(all_LO_records[i]), all_LO_records[i], orientation = :horizontal, label = "LO events", xlabel = "events", ylabel = "", title = "", legend = :bottomright, xlims = (0, 10), ylims = (0, 100), alpha = 0.5, color = :blue, yflip=true)
    bar!(1:length(all_LIC_records[i]), all_LIC_records[i], orientation = :horizontal, label = "LORO events", xlabel = "events", ylabel = "", legend = :bottomright, xlims = (0, 10), ylims = (0, 100), alpha = 0.5, color = :red, yflip=true)
...
savefig(final_plot, figure_file_path)
    ...
    for i in 1:6
        ...
        for startbacteria in 1:15   
            println(i, "start bacteria: ", startbacteria)
