...
mutable struct StateE
    ...
    Eta::Float64 # adsorption rate
end


# Main function
function simulate_population_agents(states::Vector{StateE}, time_step, record_time_step, final_time, bacteria, phage, infected, volume, 
    growth_rate, nutrient, lysis_rate, burst_rate, eclipse, growth_timer, lysis_timer, mean_eta, sd_eta; lysis_inhibition=false, lysis_inhibition_timer=5, 
    ...
    while timenow < final_time
        ...
        for i in 1:bacteria
            #println("phage infection", states[i].Eta)
            phageinfect= rand(Poisson(states[i].Eta*phage/volume*time_step))
            #println("phage infection",phageinfect)
            phage_loss+=phageinfect
            if phageinfect > 0
        ...
        #Add the new bacteria, number new_bacteria
            for _ in 1:new_bacteria
                states = push!(states, StateE(1, 0, 0.0, false, rand(LogNormal(mu_eta, sigma_eta))))
            end
        end
...
if culture_growth
    ...
    time, Btimeseries, Itimeseries, Ptimeseries, lysis_time_record, states, bacteria, phage = simulate_population_agents(
        ...
        lysis_rate, burst_rate, eclipse, growth_timer, lysis_timer, mean_eta, sd_eta; 
    ...
    P0 = msoi * ((Float64(bacteria) / volume)) / (1 - exp(-mean_eta * Float64(bacteria) / volume * si_duration))
    si_time = 15. # minutes
    ...
    # Create directories if they do not exist
    data_dir = "data_files_struct_growth_LOR_etadist"
    figures_dir = "figure_files_struct_growth_LOR_etadist"
    mkpath(data_dir)
    mkpath(figures_dir)
    ...
    sigma_eta = sqrt(log(sd_eta^2 / mean_eta^2 + 1))
    for i in 1:bacteria
        states[i].Eta = rand(LogNormal(mu_eta, sigma_eta))
    end
