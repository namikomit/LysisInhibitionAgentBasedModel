...
mutable struct State
    ...
    Astate::UInt16 # number of active "antiholin" that cancels the lysis timer
    ...
    while timenow < final_time
        ...
        #Add the new bacteria, number new_bacteria
            for _ in 1:new_bacteria
                states = push!(states, State(1, 0, 0, 0.0, false))
            end
        end
...
if culture_growth
    ...
    states = [State(initial_values[i], 0, 0, 0.0, false) for i in 1:bacteria]


    # Create directories if they do not exist
    data_dir = "data_files_struct_culture_antiholin"
    figures_dir = "figure_files_struct_culture_antiholin"
    mkpath(data_dir)
    mkpath(figures_dir)
    ...
    # Create directories if they do not exist
    data_dir = "data_files_struct_growth_LOR_antiholin"
    figures_dir = "figure_files_struct_growth_LOR_antiholin"
    mkpath(data_dir)
    mkpath(figures_dir)
